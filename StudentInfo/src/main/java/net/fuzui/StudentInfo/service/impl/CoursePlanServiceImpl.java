...
public class CoursePlanServiceImpl implements CoursePlanService {
    ...
    @Override
    public boolean updateCredits(String sid, String cid) {
        Map<String,Object> data = new HashMap<String,Object>();
        data.put("cid",cid);
        List<Student> byStuSid = selectCourseMapper.getByStuSid(data);
        boolean b = false;
        for (Student student : byStuSid) {
            if (student.getSid().equals(sid)) {
                if (student.getCredits()+1 <= student.getType() ){
                     b = coursePlanMapper.updateCredits(student.getCredits() + 1, sid, cid);
                }
            }
        }
        return b;
